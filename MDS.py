...
def get_ordination(sample_size):
    dist_matrix = pd.read_csv('../KCL-Content/MDS_data/pw_dist_' + sample_size + '.csv', sep='\t')
    mds = MDS(dissimilarity='precomputed', random_state=0)
    ordination = mds.fit_transform(dist_matrix)
    pd.DataFrame(ordination).to_csv('../KCL-Content/MDS_data/ordination_' + sample_size + '.csv', sep='\t', index=False)


def plot_ordination(var, sample_size):
    ordination = pd.read_csv('../KCL-Content/MDS_data/ordination_' + sample_size + '.csv', sep='\t')
    pw_meta = pd.read_csv('../KCL-Content/MDS_data/meta_' + sample_size + '.csv', sep='\t')
...
def plot_ordination_sizes(var, sample_size):
    ...
    pw_meta['peptide'] = [re.sub('Pool-[0-9]*', 'peptide', i) for i in pw_meta['peptide']]
